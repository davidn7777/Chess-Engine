'''
Non-notebook version of the first it eration of AB pruning (ChessEngineAB.ipynb)
'''
import pygame
import sys
import chess
...
#Functions for parsin FEN to ML predict
...
def alpha_beta_search(board, depth, alpha, beta, maximizing_player,model):

    #pygame.event.pump()
    if depth == 0 or board.is_game_over():
        return evaluateFEN(board,model), None
    ...
    if maximizing_player:
        ...
        for move in board.legal_moves:
            board.push(move)
            cur_value, _ = alpha_beta_search(board, depth - 1, alpha, beta, False, model)
            board.pop()
            if cur_value > value:
                value = cur_value
                best_move = move
        ...
        for move in board.legal_moves:
            board.push(move)
            cur_value, _ = alpha_beta_search(board, depth - 1, alpha, beta, True, model)
            board.pop()
            if cur_value < value:
                value = cur_value
                best_move = move
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if len(select_move) == 4:
                    if chess.Move.from_uci(select_move) in board.legal_moves:
                        ...
                        count = 0
                        value, move = alpha_beta_search(board,3,-float('inf'),float('inf'), False,model)
                        print(value, str(move))
